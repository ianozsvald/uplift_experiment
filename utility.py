...
def determine_churners(ppl, marketing_prop, seed=2):
    ...
    print(
        f"determine_churners on {ppl.shape[0]} rows with marketing_prop {marketing_prop:0.2f}"
    )
    ppl = ppl.copy()
    rng = np.random.default_rng(seed=seed)
    ...
    ppl["will_churn"] = ppl["prob_churn"].apply(
        lambda p: rng.binomial(1, p)
    )  # works seed 1, 0 causes bug
    return ppl
